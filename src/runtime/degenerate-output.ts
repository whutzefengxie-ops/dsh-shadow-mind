 ...
 */
export const MAX_REASONING_CHARS_BASE = 96_000

/** Reasoning-budget multiplier per adapter-owned reasoning-effort id. */
const REASONING_EFFORT_MULTIPLIERS: Readonly<Record<string, number>> = {
  low: 1,
  medium: 1,
  high: 2,
  max: 4,
 ...
 */
export function resolveReasoningBudget(effort: string | undefined): number {
  const multiplier = effort === undefined
    ? UNKNOWN_EFFORT_MULTIPLIER
    : REASONING_EFFORT_MULTIPLIERS[effort] ?? UNKNOWN_EFFORT_MULTIPLIER
...
export class DegenerateOutputGuard {
   ...
   */
  constructor(reasoningBudget: number = MAX_REASONING_CHARS_BASE) {
