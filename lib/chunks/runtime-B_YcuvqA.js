...
function attachThinkFirst(childCtx, activationBoundary, skipSteer) {
	childCtx.on("system-prompt/assemble", async (_assembly, _context, next) => {
		...
		return childAgent.session.snapshotEvents().some((event) => event.seq >= activationBoundary && event.type === "assistant/message") ? transformed : {
			...transformed,
			tools: []
		};
	});
...
*/
function attachDegenerateOutputGuard(childCtx, childId, state, reasoningBudget) {
	...
	childCtx.on("session/event", (session, event) => {
		...
		if (event.type === "assistant/attempt") {
			for (const record of event.data.stream) if (record.type === "text-chunks") for (const text of record.texts) {
			...
			else if (record.type === "reasoning-chunks") for (const text of record.texts) {
...
async function startInProcessRun(request) {
	...
	const setup = (childCtx, child) => {
		...
		attachDegenerateOutputGuard(childCtx, childId, degenerateState, resolveReasoningBudget(request.modelSelection?.reasoningEffort));
