...
function defaultSeed(legacy) {
	...
	return {
		...
		...first.timeoutSeconds === void 0 ? {} : { timeoutSeconds: first.timeoutSeconds },
		tools: [...first.tools],
	...
	return class ShadowMindRuntime extends _classSuper {
		...
		*/
		...
		async scheduleTurn(agent, state, cycle, epoch, capturedThroughSeq) {
			...
			if (!shouldRunShadow(Math.min(1, definition.activationProbability * (state.decayFactors.get(definition.id) ?? 1)), this.random)) return;
...
//#endregion
export { optionalModelRoute as A, DEFAULT_SHADOW_PROMPT as C, DEFAULT_SHADOW_ID as D, parseShadowDefinition as E, Config as O, seededRandom as S, ShadowRegistry as T, buildShadowPrompt as _, PROBE_CLASSES_V1 as a, summarizeToolResult as b, classifyChallenge as c, buildShadowModelCatalog as d, containsHoldoutLiteral as f, ReportBatcher as g, vendorFamily as h, PERSONA_AFFINITIES as i, SHADOW_MODEL_ROUTE_PATTERN as k, classifyChallengeObservation as l, resolveIndependence as m, SHADOW_MIND_SETTINGS_NAMESPACE as n, renderProbeChecklist as o, redactHoldoutLiterals as p, ShadowMindRuntime as r, detectPatterns as s, DEFAULT_SHADOW_TOOLS as t, observeChallenge as u, projectTrajectory as v, SHADOW_ID_PATTERN as w, shouldRunShadow as x, projectTrajectoryWithAnchors as y };
