...
*/
function trimTrajectoryToBudget(trajectory, budget) {
	const contentBudget = budget - 59 - 2;
...
function buildShadowPrompt(definition, trajectory, capturedThroughSeq, maxPromptChars) {
	...
	if (maxPromptChars > 0) {
		...
		if (body.length > budget) body = budget > 0 ? trimTrajectoryToBudget(body, budget) : "[no model-visible trajectory content]";
	}
	return `${header}\n${body}`;
}
//#endregion
...
const SHADOW_PROMPT_CHARS_PER_TOKEN = 4;
/**
...
*/
const SHADOW_PROMPT_RESERVE_CHARS = 16384;
/**
...
*/
async function resolveModelPromptCapChars(ctx, route) {
	...
	try {
		const window = (await llm.resolveModelInfo(route.slice(0, slash), route.slice(slash + 1)))?.context?.contextWindow;
		if (window === void 0 || !Number.isFinite(window) || window <= 0) return 0;
		return Math.max(0, Math.floor(window * 4) - SHADOW_PROMPT_RESERVE_CHARS);
	} catch {
		return 0;
	}
}
...
*/
function effectivePromptCapChars(userBound, modelCap) {
...
let ShadowMindRuntime = (() => {
	...
	return class ShadowMindRuntime extends _classSuper {
		...
		*/
		latestFailedRun(agent) {
			...
			for (const cycle of state.cycles.values()) for (const entry of cycle.runs) {
				...
				if (latest === void 0 || entry.view.capturedThroughSeq > latest.capturedThroughSeq || entry.view.capturedThroughSeq === latest.capturedThroughSeq && entry.view.startedAt > latest.startedAt) latest = entry.view;
			}
			return latest;
		}
		/**
		...
		* @returns Status after the retry was admitted.
		*/
		async retryLatest(agent) {
			const failed = this.latestFailedRun(agent);
			if (failed === void 0) throw new Error("this session has no failed or aborted Shadow run to retry");
		...
		*/
		async reviewNow(agent) {
			this.assertRoot(agent);
			const state = this.owner(agent);
			if (state.totalRuns > 0) throw new Error(`this session has already admitted ${state.totalRuns} Shadow run(s); use /shadow retry to rerun a failed review`);
		...
		async runShadow(agent, state, entry, definition) {
			...
			try {
				...
				const modelCap = await resolveModelPromptCapChars(this.ctx, shadowModelRoute(definition, agent, entry.frugalRoute));
				prompt = redactHoldoutLiterals(buildShadowPrompt(definition, projection.text, entry.capturedThroughSeq, effectivePromptCapChars(settings.maxPromptChars, modelCap)), holdoutKeys);
...
//#endregion
export { parseShadowDefinition as A, projectTrajectoryWithAnchors as C, DEFAULT_SHADOW_PROMPT as D, seededRandom as E, Config as M, SHADOW_MODEL_ROUTE_PATTERN as N, SHADOW_ID_PATTERN as O, optionalModelRoute as P, projectTrajectory as S, shouldRunShadow as T, redactHoldoutLiterals as _, PROBE_CLASSES_V1 as a, ReportBatcher as b, classifyChallenge as c, SHADOW_PROMPT_CHARS_PER_TOKEN as d, SHADOW_PROMPT_RESERVE_CHARS as f, containsHoldoutLiteral as g, buildShadowModelCatalog as h, PERSONA_AFFINITIES as i, DEFAULT_SHADOW_ID as j, ShadowRegistry as k, classifyChallengeObservation as l, resolveModelPromptCapChars as m, SHADOW_MIND_SETTINGS_NAMESPACE as n, renderProbeChecklist as o, effectivePromptCapChars as p, ShadowMindRuntime as r, detectPatterns as s, DEFAULT_SHADOW_TOOLS as t, observeChallenge as u, resolveIndependence as v, summarizeToolResult as w, buildShadowPrompt as x, vendorFamily as y };
