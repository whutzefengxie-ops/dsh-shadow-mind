...
const DEFAULT_COLLAPSED_BY_DEFAULT = true;
//#endregion
...
*/
const MAX_REASONING_CHARS_BASE = 96e3;
/** Reasoning-budget multiplier per adapter-owned reasoning-effort id. */
const REASONING_EFFORT_MULTIPLIERS = {
	low: 1,
	medium: 1,
	high: 2,
	max: 4
...
/**
* Multiplier used when the child's reasoning effort is unknown (no per-run
* model selection): assume the heaviest tier so a healthy inherited max-effort
* run can never be misclassified by the reasoning budget.
*/
const UNKNOWN_EFFORT_MULTIPLIER = 4;
/**
...
*/
function resolveReasoningBudget(effort) {
	const multiplier = effort === void 0 ? UNKNOWN_EFFORT_MULTIPLIER : REASONING_EFFORT_MULTIPLIERS[effort] ?? UNKNOWN_EFFORT_MULTIPLIER;
	return MAX_REASONING_CHARS_BASE * multiplier;
}
...
var DegenerateOutputGuard = class {
	...
	*/
	constructor(reasoningBudget = MAX_REASONING_CHARS_BASE) {
	...
	*/
	observeChunk(text, kind) {
		if (this.fired !== void 0 || text === "") return void 0;
		if (kind === "text") {
			this.textSinceReset += text.length;
			if (this.textSinceReset > 96e3) {
				this.fired = "output-budget";
				return { reason: "output-budget" };
			}
		} else {
			this.reasoningSinceReset += text.length;
			if (this.reasoningSinceReset > this.reasoningBudget) {
				this.fired = "output-budget";
				return { reason: "output-budget" };
			}
		}
...
* @param child - the unpublished Agent handed to setup; the composition boundary
*   runs before publication, so the registry must never be consulted here.
*/
function attachThinkFirst(childCtx, child, activationBoundary, skipSteer) {
	childCtx.on("system-prompt/assemble", async (_assembly, _context, next) => {
		const transformed = await next();
		return child.session.snapshotEvents().some((event) => event.seq >= activationBoundary && event.type === "assistant/message") ? transformed : {
			...transformed,
			tools: []
		};
	});
...
* into a repeated-token loop or an unbounded output flood.
*
* The live `agent/assistant-stream` frames are the only pre-settlement view of
* the stream: the durable `assistant/attempt` event commits once the whole
* attempt has already streamed, which is far too late to cut a runaway loop
* short. Frames are agent-scoped, so this listener sees exactly the child's own
* chunks; the durable `session/event` bus still supplies the progress signals
* (tool calls and step/turn boundaries) that reset the per-stretch budgets.
* @param child - the unpublished Agent handed to setup. `ctx.agents` cannot be
*   queried during setup: the factory awaits setup BEFORE inserting or announcing
*   the session or agent, so a registry lookup here is always empty.
* @param reasoningBudget - the child's reasoning-channel budget, resolved from
*   its reasoning effort.
*/
function attachDegenerateOutputGuard(childCtx, child, childId, state, reasoningBudget) {
	...
	childCtx.on("agent/assistant-stream", ({ agent, frame }) => {
		if (state.reason !== void 0 || agent.id !== childId) return;
		if (frame.type === "end") {
		...
		if (detection !== void 0) fire(detection.reason);
	});
	childCtx.on("session/event", (session, event) => {
		...
		if (event.type === "step/end" || event.type === "turn/end") guard.observeBoundary();
	});
}
/**
...
async function startInProcessRun(request) {
	...
	const setup = (childCtx, child) => {
		...
		if (request.thinkFirst === true) attachThinkFirst(childCtx, child, activationBoundary, () => degenerateState.reason !== void 0);
		attachDegenerateOutputGuard(childCtx, child, childId, degenerateState, resolveReasoningBudget(request.modelSelection?.reasoningEffort));
...
const SHADOW_MIND_SETTINGS_NAMESPACE = "shadow-mind";
...
function deliberationLength(events) {
	...
	for (const event of events) {
		...
		if (event.type === "assistant/attempt") {
			for (const record of event.data.stream) if (record.type === "text-chunks") chars += record.texts.reduce((sum, text) => sum + text.length, 0);
			else if (record.type === "reasoning-chunks") chars += record.texts.reduce((sum, text) => sum + text.length, 0);
		...
		*/
		...
		evaluateValueChallenges(agent, state, observedThroughSeq) {
			...
			for (const challenge of state.pendingChallenges.values()) {
				const classification = classifyChallenge(agent.session.snapshotEvents(), challenge, this.settingsValue.valueLoopWindowTurns);
		...
		async runShadow(agent, state, entry, definition) {
			...
			const effectiveToolNames = [.../* @__PURE__ */ new Set([...DEFAULT_SHADOW_TOOLS, ...definition.tools])];
			...
			try {
				...
				projection = projectTrajectoryWithAnchors(agent.session.snapshotEvents(), entry.capturedThroughSeq, settings.argumentDisclosure, definition.capture);
				...
				deliberationChars = run.localAgent === void 0 ? 0 : deliberationLength(run.localAgent.session.snapshotEvents());
				childTools = run.localAgent === void 0 ? void 0 : toolTelemetry(run.localAgent.session.snapshotEvents());
			} catch (error) {
...
//#endregion
export { shouldRunShadow as A, optionalModelRoute as B, projectTrajectory as C, SHADOW_PROMPT_RESERVE_TOKENS as D, SHADOW_PROMPT_NON_CJK_CHARS_PER_TOKEN as E, parseShadowDefinition as F, Config as I, DEFAULT_COLLAPSED_BY_DEFAULT as L, DEFAULT_SHADOW_PROMPT as M, SHADOW_ID_PATTERN as N, estimateTextTokens as O, ShadowRegistry as P, DEFAULT_SHADOW_ID as R, buildShadowPrompt as S, summarizeToolResult as T, MAX_CHARS_WITHOUT_TOOL_CALL as _, PROBE_CLASSES_V1 as a, resolveReasoningBudget as b, classifyChallenge as c, buildShadowModelCatalog as d, containsHoldoutLiteral as f, DegenerateOutputGuard as g, vendorFamily as h, PERSONA_AFFINITIES as i, seededRandom as j, resolveModelPromptTokenBudget as k, classifyChallengeObservation as l, resolveIndependence as m, SHADOW_MIND_SETTINGS_NAMESPACE as n, renderProbeChecklist as o, redactHoldoutLiterals as p, ShadowMindRuntime as r, detectPatterns as s, DEFAULT_SHADOW_TOOLS as t, observeChallenge as u, MAX_REASONING_CHARS_BASE as v, projectTrajectoryWithAnchors as w, ReportBatcher as x, hasRepeatedSuffix as y, SHADOW_MODEL_ROUTE_PATTERN as z };
