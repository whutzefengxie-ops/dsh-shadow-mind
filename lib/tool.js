import { j as DEFAULT_SHADOW_ID } from "./chunks/runtime-CZpsFGKX.js";
...
function admittedConfirmation(status) {
	const runs = status.active.map((entry) => `${entry.shadowId}/${entry.runId}`);
	return runs.length === 0 ? "Shadow review acknowledged; no run is active." : `Shadow review admitted; ${String(runs.length)} running: ${runs.join(", ")}.`;
}
...
function apply(ctx) {
	ctx.commands.register({
		...
		handler: async ({ agent, rawInput }) => {
			if (rawInput.trim() !== "new") return {
				kind: "error",
				text: "Usage: /shadow new"
			};
			try {
				return {
					kind: "success",
					text: admittedConfirmation(await ctx.shadowMind.reviewNow(agent))
				};
			} catch (error) {
				return {
					kind: "error",
