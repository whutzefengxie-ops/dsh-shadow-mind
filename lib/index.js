...
function buildShadowPrompt(definition, trajectory, capturedThroughSeq, maxPromptChars) {
	...
	if (maxPromptChars > 0 && prompt.length > maxPromptChars) throw new Error(`shadow prompt has ${String(prompt.length)} characters, above maxPromptChars ${String(maxPromptChars)}`);
	return prompt;
}
//#endregion
...
function buildSynthesisPrompt(definition, conflict, maxChars) {
	...
	if (maxChars > 0 && prompt.length > maxChars) throw new Error(`Shadow synthesis prompt length ${String(prompt.length)} exceeds maxPromptChars ${String(maxChars)}`);
	return prompt;
}
//#endregion
	...
	return class ShadowMindRuntime extends _classSuper {
		...
		*/
		...
		async runShadow(agent, state, entry, definition) {
			...
			if (reportContent === "" || settings.maxReportChars > 0 && reportContent.length > settings.maxReportChars || entry.childSessionId === void 0) {
				await this.finishRun(state, entry, "failed", {
					stage: "validate",
		...
		async synthesizeConflict(agent, state, accepted, conflict) {
			...
			if (content === "" || this.settingsValue.maxReportChars > 0 && content.length > this.settingsValue.maxReportChars || containsHoldoutLiteral(content, reportKeys)) {
		...
		*/
		...
		buildGateJudgePrompt(settings, agent, command) {
			...
			if (bound <= 0) return `${critical}\n\n## Recent root trajectory\n${trajectory}`;
			if (critical.length > bound) throw new Error(`command gate judge prompt needs ${String(critical.length)} characters for its rules and command, above maxPromptChars ${String(bound)}`);
			const available = bound - critical.length - 26;
