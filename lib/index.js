...
const DEFAULT_HEARTBEAT_PROBABILITY = 1 / 3;
/** Default maximum framed prompt size. */
const DEFAULT_MAX_PROMPT_CHARS = 12e4;
/** Default maximum accepted report size. */
const DEFAULT_MAX_REPORT_CHARS = 2e4;
/** Default result size that activates the long-output review boost. */
const DEFAULT_LONG_OUTPUT_BOOST_CHARS = 5e4;
/** Default minimum novel-envelope share. */
const DEFAULT_DIMINISHING_NOVELTY_THRESHOLD = .4;
/** Commands matching one of these are denied before any judge runs. */
const DEFAULT_COMMAND_GATE_DENY_PATTERNS = [
	"\\bStop-Process\\b",
	"\\bStop-Service\\b",
	"\\bRestart-Service\\b",
	"\\bSuspend-Service\\b",
...
*/
const DEFAULT_COMMAND_GATE_ALLOW_PATTERNS = [
...
const COMMAND_GATE_SEPARATOR_PATTERN = /[;&|`\r\n]/u;
...
const SHADOW_MIND_SETTINGS_OBJECT = z.object({
	...
	commandGateScope: z.union(["root-only", "root-and-subagents"]).default("root-only"),
	...
	commandGateJudgeTimeoutSeconds: z.number().min(.001).default(30),
	commandGateOnJudgeFailure: z.union(["deny", "allow"]).default("deny"),
	commandGateMaxParallel: z.number().step(1).min(1).default(1),
	commandGateVerdictTtlSeconds: z.number().min(0).default(120)
});
...
function parseShadowDefinition(source, sourcePath) {
	...
	if (agentPreset !== void 0 && !PRESET_ID_PATTERN.test(agentPreset)) throw new Error("agent_preset must match a DSH preset id (lowercase letters, digits, and dashes)");
...
function serializeDefinition(definition) {
	...
	if (definition.agentPreset !== void 0) metadata["agent_preset"] = definition.agentPreset;
...
function editable(definition) {
	return {
		...
		...definition.agentPreset === void 0 ? {} : { agentPreset: definition.agentPreset },
...
function updatedDefinition(current, patch) {
	...
	return {
		...
		...merged.agentPreset === void 0 ? {} : { agentPreset: merged.agentPreset },
...
//#endregion
//#region .build/runtime/command-gate.js
/**
...
function emptyCommandGateStats() {
	return {
		denies: 0,
		allows: 0,
		judgeRuns: 0,
...
var CommandGate = class {
	...
	interested(exec) {
		...
		if (!settings.commandGateEnabled || exec.agent === void 0) return void 0;
		if (!settings.commandGateTools.includes(exec.name)) return void 0;
		if (settings.commandGateScope === "root-only" && !this.runtime.isRoot(exec.agent)) return void 0;
		const args = exec.arguments;
		if (typeof args.command !== "string" || args.command.trim() === "") return void 0;
		return {
	...
	async decide(exec, command) {
		...
		if (this.allows(settings, command)) {
			const verdict = {
				allow: true,
				reason: "matches a command-gate allow pattern",
				tier: "allow-pattern"
			};
		...
		if (cached !== void 0 && cached.until > Date.now()) {
			const verdict = {
				...cached.verdict,
				tier: "cached"
			};
		...
		const started = (async () => {
			try {
				...
				return {
					allow: false,
					reason: "command gate judge wait aborted",
					tier: "failure"
				};
			}
			try {
				...
				if (outcome.kind === "verdict") {
					...
					this.cache.set(key, {
						verdict,
						until: Date.now() + settings.commandGateVerdictTtlSeconds * 1e3
					});
					this.log(agent, exec, command, verdict);
					return verdict;
				}
	...
	*/
	async raceJudge(agent, command, signal, settings) {
		let timer;
		const onAbort = () => {
		...
		if (signal.aborted) return {
			kind: "failure",
			reason: "judge aborted with the root turn"
		};
		signal.addEventListener("abort", onAbort, { once: true });
		const timeout = new Promise((resolve) => {
			...
			}, settings.commandGateJudgeTimeoutSeconds * 1e3);
		});
		try {
	...
	denyVerdict(settings, command) {
		for (const pattern of settings.commandGateDenyPatterns) {
			...
			return {
				allow: false,
				reason: protectedTarget === void 0 ? `blocked by Shadow Mind command gate: the command matches a deny pattern (${pattern})` : `blocked by Shadow Mind command gate: the command targets protected ${protectedTarget.kind} "${protectedTarget.name}"`,
				tier: "deny-pattern"
			};
		}
	}
	/** Named protected process or service the command mentions, when one does. */
	protectedTarget(settings, command) {
		const folded = command.toLowerCase();
		for (const name of settings.commandGateProtectedProcesses) if (name !== "" && folded.includes(name.toLowerCase())) return {
			kind: "process",
			name
		};
		for (const name of settings.commandGateProtectedServices) if (name !== "" && folded.includes(name.toLowerCase())) return {
			kind: "service",
			name
		};
	}
	/** Deterministic read-only allowance. */
	allows(settings, command) {
	...
	acquire(signal) {
		if (signal.aborted) return Promise.reject(/* @__PURE__ */ new Error("aborted while queued for the command gate judge"));
		...
		return new Promise((resolve, reject) => {
			...
			const onAbort = () => {
				...
				signal.removeEventListener("abort", onAbort);
				reject(/* @__PURE__ */ new Error("aborted while queued for the command gate judge"));
			};
			signal.addEventListener("abort", onAbort, { once: true });
	...
	log(agent, exec, command, verdict) {
		try {
...
async function buildShadowModelCatalog(ctx) {
	...
	if (presets !== void 0) try {
		...
		for (const preset of listed) agentPresets.push({
			id: preset.id,
...
function parseGateOutput(value) {
	if (value === null || typeof value !== "object" || Array.isArray(value)) return void 0;
	const record = value;
...
function personaFromComposition(composition) {
	if (!Array.isArray(composition)) return void 0;
...
function editableDefinition(input) {
	return {
		...
		agentPreset: input.agentPreset ?? void 0,
...
let ShadowMindRuntime = (() => {
	...
	return class ShadowMindRuntime extends _classSuper {
		...
		*/
		...
		async runShadow(agent, state, entry, definition) {
			...
			try {
				...
				const persona = presetId === void 0 ? void 0 : await this.resolveAgentPresetPersona(presetId);
		...
		async synthesizeConflict(agent, state, accepted, conflict) {
			...
			try {
				...
				const selection = modelSelection(definition, this.settingsValue, agent, {
					...this.settingsValue.synthesisModel === void 0 ? {} : { route: this.settingsValue.synthesisModel },
					...frugalRoute === void 0 ? {} : { route: frugalRoute },
					...this.settingsValue.synthesisReasoningEffort === void 0 ? {} : { effort: this.settingsValue.synthesisReasoningEffort }
				});
				assertConditioningCapabilities(this.ctx, {
					modelSelection: selection !== void 0,
					minimalContext: definition.context === "minimal",
				...
				const presetId = this.settingsValue.synthesisAgentPreset ?? definition.agentPreset ?? this.settingsValue.defaultAgentPreset;
				const persona = presetId === void 0 ? void 0 : await this.resolveAgentPresetPersona(presetId);
				run = await this.ctx.subagents.start(SHADOW_MIND_SUBAGENT_PROVIDER, {
		...
		*/
		async judgeVerdict(agent, command, signal) {
			const settings = this.settingsValue;
			...
			const timeout = setTimeout(() => controller.abort(/* @__PURE__ */ new Error("command gate judge timed out")), settings.commandGateJudgeTimeoutSeconds * 1e3);
			let run;
			try {
				const persona = presetId === void 0 ? void 0 : await this.resolveAgentPresetPersona(presetId);
				...
				run = await this.ctx.subagents.start(SHADOW_MIND_SUBAGENT_PROVIDER, {
					...
					prompt: [{
						type: "text",
						text: this.buildGateJudgePrompt(settings, agent, command)
					}],
					signal: controller.signal,
				...
				const output = result.stopReason === "completed" ? parseGateOutput(result.structured) : void 0;
				if (output === void 0) {
					const tail = run.localAgent?.session.events.slice(-4).map((event) => `${event.type}:${JSON.stringify(event.data).slice(0, 220)}`).join(" | ");
					this.ctx.logger.warn("dsh-shadow-mind: command-gate judge produced no valid verdict (stop reason %s): %s", result.stopReason, tail ?? "no child session");
					return {
						kind: "failure",
						reason: `judge produced no valid verdict (${result.stopReason}${tail === void 0 ? "" : `; ${tail}`})`
					};
				}
				return {
					kind: "verdict",
		...
		buildGateJudgePrompt(settings, agent, command) {
			const header = [
			...
			if (settings.commandGateContext !== void 0 && settings.commandGateContext.trim() !== "") protectedLines.push(settings.commandGateContext.trim());
			if (settings.commandGateProtectedProcesses.length > 0) protectedLines.push(`Protected processes: ${settings.commandGateProtectedProcesses.join(", ")}`);
			if (settings.commandGateProtectedServices.length > 0) protectedLines.push(`Protected services: ${settings.commandGateProtectedServices.join(", ")}`);
			...
			const prefix = `${header}\n\n${[
				...
				...command.workdir === void 0 ? [] : [`workdir: ${command.workdir}`],
				"",
				"## Environment",
				`workspace: ${agent.session.header.cwd ?? "unknown"}`,
				tailChars(environment, Math.max(0, settings.maxPromptChars - 2e3))
			].join("\n")}\n\n## Recent root trajectory`;
			const available = settings.maxPromptChars - prefix.length - 2;
			if (available <= 0) return tailChars(prefix, settings.maxPromptChars - 2);
			const lastSeq = agent.session.events.at(-1)?.seq ?? 0;
			return `${prefix}\n${tailChars(projectTrajectory(agent.session.events, lastSeq, settings.argumentDisclosure, "full"), available)}`;
		}
		/** Append one gate diagnostic record without letting storage failures escape. */
		appendGateLog(agent, record) {
			this.registry.appendDebug("command-gate", {
				rootSessionId: agent.id,
				...record
			}).catch((error) => {
		...
		*/
		async resolveAgentPresetPersona(presetId) {
			const presets = this.ctx.get("agentPresets");
			if (presets === void 0) return void 0;
			try {
				const composition = await presets.read(presetId);
				return personaFromComposition(parse(composition));
			} catch (error) {
...
//#endregion
export { CommandGate, Config, DEFAULT_SHADOW_TOOLS, GATE_OUTPUT_SCHEMA, PERSONA_AFFINITIES, PROBE_CLASSES_V1, ReportBatcher, SHADOW_ID_PATTERN, SHADOW_MIND_SETTINGS_NAMESPACE, SHADOW_MODEL_ROUTE_PATTERN, ShadowMindRuntime, ShadowMindRuntime as default, ShadowRegistry, boostPredicates, buildShadowModelCatalog, buildShadowPrompt, buildSynthesisPrompt, classifyChallenge, classifyChallengeObservation, containsHoldoutLiteral, detectPatterns, matchesPredicate, modelEligible, observeChallenge, optionalModelRoute, parseShadowDefinition, preferIndependentCandidates, prefilterPredicates, projectTrajectory, projectTrajectoryWithAnchors, redactHoldoutLiterals, renderProbeChecklist, resolveIndependence, seededRandom, selectShadowConflict, selectShadows, summarizeToolResult, vendorFamily };
