	...
	*/
	async holdoutKeys(id) {
		...
		try {
			...
			if (error.code === "ENOENT") throw new Error(`holdout definition ${JSON.stringify(id)} needs ${this.holdoutKeysPath}`);
			throw error;
		}
		let parsed;
		try {
		...
		*/
		...
		async runShadow(agent, state, entry, definition) {
			...
			if (output.status !== "report") {
				...
				if (typeof rawContent === "string" && rawContent.trim() !== "") this.ctx.logger.warn("dsh-shadow-mind: shadow %s returned %s with a non-empty content body; the body is not relayed and was discarded (run %s)", definition.id, output.status, entry.runId);
		...
		async debugMetadata(definition, record) {
