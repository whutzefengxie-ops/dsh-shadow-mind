...
var CommandGate = class {
	...
	async decide(exec, command) {
		...
		const started = (async () => {
			...
			try {
				stats.judgeRuns += 1;
				const outcome = await Promise.race([this.runtime.judgeVerdict(agent, command, exec.signal), this.timeoutFailure(settings, exec.signal)]);
	...
	timeoutFailure(settings, signal) {
