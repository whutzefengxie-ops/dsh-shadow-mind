...
This reference defines eight review-quality mechanisms adapted from the ouroboros lineage to the plugin's existing fresh-child scheduler. They use validated definition fields, settings, durable Session events, and bounded process-local metadata; they do not install another workflow engine.
...
Relay sections sort by severity descending. Durable provenance carries verdict, refs, optional severity, and optional `replacesRunIds`; route and vendor classification remain process-local diagnostics.
...
`preferIndependentVendor` defaults to false. When enabled, same-vendor candidates are removed only if at least two non-same-vendor candidates remain, preserving a viable review set instead of asserting purity from one reviewer.
...
Automated coverage rejects invalid probes, predicates, envelopes, definitions, settings, and holdout data while pinning every D1–D8 coordination outcome. Lifecycle and assembled-flow scenarios verify complete-relay redaction, synthesis fail-open behavior, cancellation, durable anchored provenance, same-child think-first execution, and teardown to quiescence.
