...
> **设计变更记录（合并后）**：本文档中关于 **DSH Agent 预设绑定**（`agentPreset` / `agent_preset` / persona 继承）的内容已按用户反馈**从实现中移除**——预设绑定价值边际、徒增设置页复杂度；最终交付的能力一只包含供应商 / 模型 / 思考强度三级联动绑定。下文保留原始设计以记录取舍过程，凡涉及预设的章节（1.3、2.1、2.2、2.3、4）均不再对应现行代码。
>
> **后续移除（产品收敛）**：**能力二（pwsh 命令闸门）已整体删除**，连同确定性 deny/allow 模式、法官编排与全部 `commandGate*` 设置（真实环境实测法官链路可永久卡死，后续所有受管命令在队列中被中断）；冲突综合（synthesizer）亦随「单一 Shadow 审查者」收敛删除。见 [`settings-ux-revamp.zh.md`](settings-ux-revamp.zh.md)。本文仅保留作历史设计记录。

> 范围：同一 PR 下实现两个能力。
> 能力一：所有 Shadow 子代理（评审影子、冲突综合、闸门法官）与 DSH 已配置的**供应商 / 模型 / 思考强度**绑定，设置页用下拉框选择。
