...
> **实施状态（代码完成，待 live 重启复验）**：运行时、工具面、客户端、测试与文档已全部按本方案落地并通过 `pnpm run check`。要点：单一 `default` 定义自动创建（旧定义只读保留、概率统一 70%、默认超时 600 秒）；多定义 CRUD API 删除；`synthesis.ts`、`prefilter.ts` 与 `command-gate.ts` 整体删除；设置页重构为单 Shadow 卡片 + toast + 概率滑杆（10%–100%、步进 10%、默认 70%）。**live 验收尚未完成**：新 `lib/` 已部署进 live profile（备份 `lib.bak-20260828-101939`、`lib.bak-20260828-102950-pre-600s`），但 harness 尚未重启——重启后的复验清单：会话工具面四项（`list_shadows`/`update_default_shadow`/`get_shadow_config`/`update_shadow_config`）、设置页单 Shadow 卡片（开关 + 滑杆 + 右下 toast）、旧定义只读提示、首个轮次后 `default.md`（概率 0.7、默认超时 600s）。复验通过前，对应 PR 应保持 draft / 不合并。
...
- **阶段 C（文档与测试）** ✅；`pnpm run check`（typert 校验 + typecheck + 136 测试 + 构建 + 冒烟）全绿。
