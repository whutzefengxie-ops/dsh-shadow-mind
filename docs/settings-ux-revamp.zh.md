...
> **实施状态（已完成）**：运行时、工具面、客户端、测试与文档已全部按本方案落地并通过 `pnpm run check`。要点：单一 `default` 定义自动创建（旧定义只读保留、概率统一 70%、默认超时 600 秒）；多定义 CRUD API 删除；`synthesis.ts`、`prefilter.ts` 与 `command-gate.ts` 整体删除；设置页重构为单 Shadow 卡片 + toast + 概率滑杆（10%–100%、步进 10%、默认 70%）。
...
- **阶段 C（文档与测试）** ✅；`pnpm run check`（typert 校验 + typecheck + 135 测试 + 构建 + 冒烟）全绿。
