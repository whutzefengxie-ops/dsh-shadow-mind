...
> **Implementation status (complete; live acceptance passed)**: runtime, tool surface, client, tests, and docs all follow this plan and pass `pnpm run check`, and the live end-to-end acceptance is done. Highlights: a single `default` definition is auto-created (legacy definitions kept read-only, probability defaults to 70%, default timeout 600 seconds); the admin `catalog` remote idempotently ensures `default.md`, so a fresh installation shows an editable card instead of the empty-state false alarm ("Shadow Mind data is unavailable"); the multi-definition CRUD API is deleted; `synthesis.ts`, `prefilter.ts`, and `command-gate.ts` are deleted outright; the settings page is rebuilt as a single Shadow card with toasts and a 10%–100% probability slider (step 10%, default 70%). **Live acceptance progress**: the harness has been restarted and is running the new build — the four-tool surface (`list_shadows`/`update_default_shadow`/`get_shadow_config`/`update_shadow_config`) is live, `get_shadow_config` reads default timeout `600` with none of the removed keys present, and `list_shadows` shows `default` auto-created (probability 0.7 by default, editable via the settings slider, e.g. 0.8 now persisted) with the legacy `implementation-reviewer` kept read-only and never scheduled and `diagnostics` empty; the new `lib/` is deployed into the live profile (backups listed below). **Acceptance (complete)**: the harness was restarted with the new build and the browser single-card settings page, toasts, and the timeout default `600` ("Empty uses the global default: 600s (10 minutes).") are all confirmed; the client tolerates an old-host-shaped snapshot via the optional `defaultShadowTimeoutSeconds` field (verified by actually parsing such a snapshot), so client/host version skew no longer causes a full-page decode error.
