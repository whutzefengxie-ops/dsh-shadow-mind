...
> **Implementation status (code complete; server-side live acceptance passed, only the client render confirmation remains)**: runtime, tool surface, client, tests, and docs all follow this plan and pass `pnpm run check`. Highlights: a single `default` definition is auto-created (legacy definitions kept read-only, probability defaults to 70%, default timeout 600 seconds); the admin `catalog` remote idempotently ensures `default.md`, so a fresh installation shows an editable card instead of the empty-state false alarm ("Shadow Mind data is unavailable"); the multi-definition CRUD API is deleted; `synthesis.ts`, `prefilter.ts`, and `command-gate.ts` are deleted outright; the settings page is rebuilt as a single Shadow card with toasts and a 10%–100% probability slider (step 10%, default 70%). **Live acceptance progress**: the harness has been restarted and is running the new build — the four-tool surface (`list_shadows`/`update_default_shadow`/`get_shadow_config`/`update_shadow_config`) is live, `get_shadow_config` reads default timeout `600` with none of the removed keys present, and `list_shadows` shows `default` auto-created (probability 0.7 by default, editable via the settings slider, e.g. 0.8 now persisted) with the legacy `implementation-reviewer` kept read-only and never scheduled and `diagnostics` empty; the new `lib/` is deployed into the live profile (backups listed below). **The only remaining item**: the user restarts the harness (so the host loads the new catalog contract) and then confirms in the browser (Ctrl+F5) that the timeout field renders the `600` placeholder and the "Empty uses the global default: 600s (10 minutes)." hint. Before that restart the client degrades gracefully (the new decoder was verified by actually parsing an old-host-shaped snapshot missing `defaultShadowTimeoutSeconds`, so it does not full-screen error) but does not show the exact number; showing the number depends on the host restart. Until that passes, the corresponding PR must stay draft / not merge.

> **Timeout root cause and effectiveness evidence**: in the old build (default timeout 300s) `implementation-reviewer.jsonl` shows every `SHADOW_TIMEOUT` firing exactly **300s** after start (five occurrences, e.g. 14:39:02→14:44:02), while completed runs took **150–294s** — so the timeouts were **budget exhaustion** at the 300s ceiling, not a hung provider or queue stall. On the new build (default 600s) `default.jsonl` records **six successful runs** (66s, 122s, 124s, 160s, 165s, 180s) with **zero timeouts**. 600s adds headroom and directly mitigates this cause; if a genuinely heavy review still exceeds 600s, a future 600s-ceiling timeout would appear in the logs, and the value can be raised then.
...
- **Phase D (live deployment and verification)**: lib/ deployed into the live profile ✅; the harness was restarted and runs the new build; server-side acceptance passed (four-tool surface, `default.md` auto-created, default timeout 600s, read-only legacy note) ✅; **the only item still open = the user confirms (after Ctrl+F5) that the timeout field renders the `600` placeholder and the hint text** ⏳.
